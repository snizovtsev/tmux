...
/* Global remote connections. */
struct remotes remotes;

static u_int	next_remote_id;
...
/* Helper functions. */
static int scanflike(3, 0) event_parse(struct event_parse_ctx *, int, const char *, ...);
...
static void	 remote_error_callback(struct bufferevent *, short, void *);
...
static void	remote_dispatch_event(struct remote *, struct evbuffer *);
...
remote_log(struct remote *r, const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	if (r->wp != NULL)
		window_remote_vadd(r->wp, 0, fmt, ap);
	va_end(ap);
}

void
remote_set_pane(struct remote *r, struct window_pane *wp, int reset)
{
	...
	if (reset)
		window_pane_reset_mode_all(r->wp);

	if ((r->wp = wp) != NULL) {
		cmd_find_from_pane(&fs, wp, 0);
		fs.r = r;
		window_pane_set_mode(wp, NULL, &window_remote_mode, &fs, NULL);
	}

}

static int
remote_cmp(struct remote *r1, struct remote *r2)
{
	return (r1->id - r2->id);
}
RB_PROTOTYPE_STATIC(remotes, remote, entry, remote_cmp);
RB_GENERATE_STATIC(remotes, remote, entry, remote_cmp);

struct remote *
remote_find_by_id(u_int id)
{
	struct remote r;

	r.id = id;
	return (RB_FIND(remotes, &remotes, &r));
}
...
remote_create(struct bufferevent *bev)
{
	...
	log_debug("%s: #%u create %%TODO", __func__, r->id);
...
static void
remote_error_callback(__unused struct bufferevent *bev, short what, void *data)
{
	struct remote	*r = data;
...
{
	char *line = EVBUFFER_DATA(buffer);

	log_debug("%s: reply=%u %s", __func__, r->reply_number, line);
	...
	if (r->reply_number) {
		...
		if (strncmp(line, "%begin ", 7) == 0)
			...
			remote_dispatch_event(r, buffer);
		else
...
static int
event_parse(struct event_parse_ctx *ctx, int args, const char *fmt, ...)
{
	va_list ap;
	int n;
...
static void
remote_dispatch_event(struct remote *r, struct evbuffer *buffer)
{
	struct event_parse_ctx ss = {
	    .line = EVBUFFER_DATA(buffer),
	    .len = EVBUFFER_LENGTH(buffer) - 1,
	};
	...
	/* %window-add @window */
	else if (event_parse(&ss, 1, "%%window-add%*1[ ]@%u", &window))
		remote_window_add(r, window);
	/* %unlinked-window-add @window */
	else if (event_parse(&ss, 1, "%%unlinked-window-add%*1[ ]@%u", &window))
	...
	/* %pause %pane */
	else if (event_parse(&ss, 1, "%%pause%*1[ ]%%%u", &pane))
		remote_pause(r, pane);
...
remote_show_environment(struct remote *r, struct environ *env, int flags)
{
	...
	while ((line = evbuffer_peek_string(reply, &n_read_out))) {
		if (line[0] != '-') {
			...
			if (flags)
				environ_set(env, line + 1, flags, "%s", "");
...
remote_window_pane_changed(struct remote *r, u_int window_id, u_int pane_id)
{
	...
	if (cp == NULL || cw == NULL) {
		/* XXX: we also observe other session notifications */
		remote_log(
...
remote_window_renamed(struct remote *r, u_int window_id, char *new_name)
{
	...
	if (cw == NULL) {
		/* XXX: we also observe other session notifications */
		remote_log(
		    r, "%s %u: no such window", __func__, window_id);
		return;
	}

	w = cw->pane->window;
	window_set_name(w, new_name);
}
...
remote_window_add(struct remote *r, u_int window_id)
{
	...
	if (cw != NULL) {
		/* XXX: we also observe other session notifications */
		remote_log(
		    r, "%s %u: already exist", __func__, window_id);
		return;
	}

	/* Sample sequence:
		%session-window-changed $0 @14 // pane queue
		%window-add @14 // pane queue
		%output %23 \033]0;sergio@winnie:/etc/fonts\007 // output queue
		%subscription-changed 666 $0 @14 5 %23 : // subscription timer
	*/

	// TODO: call list-panes for that window, blocking next requests
}
...
static void
remote_pause(struct remote *r, u_int pane_id)
{
}
